...
function rain(x,y,d,vy, acc) {
	...
	this.d = d;
	this.vy = vy;
	this.r = Math.floor(Math.random() * 205) + 50;
	this.g = Math.floor(Math.random() * 205) + 50;
	this.b = Math.floor(Math.random() * 205) + 50;
	this.acc = acc
}
...
function update() {
	...
	for (j=0; j < downpour.length; j++){
		...
		context.fillRect(downpour[j].x, downpour[j].y, downpour[j].d, downpour[j].d);
	}
